...
-- Language enum
CREATE TYPE public.debug_language AS ENUM ('python', 'c');
...
CREATE TABLE public.debug_sessions (
  ...
  language public.debug_language NOT NULL DEFAULT 'python',
  original_code TEXT NOT NULL DEFAULT '',
  analysis JSONB,
  defect_probability NUMERIC,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

GRANT SELECT, INSERT, UPDATE, DELETE ON public.debug_sessions TO authenticated;
